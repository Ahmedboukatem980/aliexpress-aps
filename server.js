...
app.post('/api/ai-refine-title', async (req, res) => {
  try {
    ...
    try {
      ...
      if (isHook) {
        prompt = `
        أنت كاتب محتوى جزائري خبير في التسويق بالعمولة (Affiliate Marketing). 
        المهمة: كتابة "هوك" (Hook) أو مقدمة قصيرة جداً ومثيرة للانتباه بالدارجة الجزائرية لمنتج AliExpress.
        
        استخدم هذه الأمثلة كمرجع لأسلوب الكتابة:
        - افاااااااااار
        ...
        - لافـــــااار بـــــاااطل الكمية محدودة
        
        القواعد:
        1. استخدم الدارجة الجزائرية البيضاء المفهومة.
        2. اجعلها قصيرة جداً (3-7 كلمات).
        3. استخدم الإيموجي المناسب.
        4. لا تكرر اسم المنتج، ركز على الإثارة والسعر.
        
        العنوان الأصلي للمنتج: ${title}
        النتيجة المطلوبة: (المقدمة فقط)
      `;
      } else {
        prompt = `
        قم بترجمة وتحسين عنوان المنتج التالي من AliExpress إلى اللغة العربية (أو الدارجة الجزائرية إذا كان ذلك مناسباً للتسويق):
        العنوان: ${title}
        
        المتطلبات:
        1. اجعل العنوان جذاباً وقصيراً.
        2. ركز على الكلمات الأساسية للمنتج.
        3. احذف الكلمات العشوائية التي يضيفها البائعون الصينيون.
        4. أضف إيموجي مناسب في البداية.
        
        النتيجة المطلوبة: (العنوان المحسن فقط)
      `;
      }
